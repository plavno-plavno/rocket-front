...
```bash
pnpm cleanup --interactive    # interactive mode
pnpm cleanup --list           # see available features
pnpm cleanup --dry-run chat   # preview before removing
pnpm cleanup kanban chat      # remove specific features
```

Run `pnpm cleanup --help` for all options (with npm, pass flags after `--`: `npm run cleanup -- --list`). The replacement files it writes live in `scripts/cleanup-templates/` as real, typechecked code. When you're done, delete `scripts/cleanup.js`, `scripts/cleanup-templates/`, and the `cleanup` entry in `package.json`.
...
Yes. Run `pnpm cleanup clerk` to remove Clerk authentication (along with organizations and billing) and wire in your own auth solution.

**How do I remove demo pages or features I don't need?**
Run `pnpm cleanup --interactive` and pick what to strip, or `pnpm cleanup --list` to see what can be removed.
