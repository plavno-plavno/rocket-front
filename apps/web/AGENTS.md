...
- **UI Components**: shadcn/ui, style `base-nova` on **Base UI** (`@base-ui/react`) — see `components.json`
- **Error Tracking**: Sentry
- **Charts**: Recharts
- **Containerization**: Docker (Node.js + pnpm)
- **Package Manager**: pnpm (workspace root `client.ui/`)
...
- shadcn/ui component library on Base UI primitives (`@base-ui/react`, style `base-nova`)
...
- oxlint (`.oxlintrc.json`) — `pnpm lint`, `pnpm lint:strict`
- oxfmt (`.oxfmtrc.json`) — `pnpm format`, `pnpm format:check`
- Husky for git hooks
...
/scripts                   # Dev tooling
    ├── cleanup.js         # Feature removal, run via `pnpm cleanup` (templates in cleanup-templates/, typechecked)
...
```bash
pnpm dlx shadcn@latest add component-name   # style base-nova (components.json); never mix in Radix variants
```
...
## Project rules (LP) — read this first

This app is the web UI of the Local Presence Platform (SDD-01). The starter's generic guidance above still applies; the rules below win on conflict.

### Where things live
- `src/features/<f>/` — one folder per feature: `feature.ts` (registry: nav, kbar, routes, status), `index.ts` (**the only** import path for other features), `api/{types,service,queries,mutations}.ts`, `components/`, `searchparams.ts`, `messages/{ru,en}.json`, `mocks/{handlers,fixtures,scenarios}.ts`, `e2e/`.
- `src/shell/` — registry-driven shell (`defineFeature`, nav builder, ScopeSelector, RealtimeProvider). `src/generated/` — generated by `pnpm gen`, gitignored, never edited.
- `src/components/lp/` — product primitives and **page templates** (`ListPage`, `DetailPage`, `AnalyticsPage`, `InboxPage`, `SettingsPage`, `WizardPage`, `PlannedPage`). Every `app/dashboard/**/page.tsx` renders one of them (`pnpm check:templates`).
- `mocks/` — mock core-api (`pnpm mock:api`, port `MOCK_PORT`). Feature handlers are auto-registered; unmocked contract routes answer 501 with the operationId.
- `packages/contracts` (`@lp/contracts`) — the OpenAPI contract and generated types.

### Data
1. `api/types.ts` **re-exports** types from `@lp/contracts` (`Schema<'X'>`, `OperationQuery<'op'>`). Hand-written DTOs are forbidden; a missing field/endpoint → `packages/contracts/CHANGE_REQUESTS.md` + a handler marked `provisional` in `mocks/scenarios.ts`.
2. HTTP only in `api/service.ts` via `coreClient()` from `@/lib/api` (enforced by dependency-cruiser). Errors are `ApiError` (RFC 9457 `code`, `fieldErrors`).
3. Queries: key factories prefixed with the feature id; every scoped query includes `scope` (`useScope()` from `@/hooks/use-scope`) in its key. Pages: `void prefetchQuery` + `HydrationBoundary` + `useSuspenseQuery`; tables: nuqs `shallow: true`.
4. Scaffold new API layers with `pnpm scaffold:api <feature> --tags <openapi tags>`; whole features with `pnpm scaffold:feature <id>` (config in `scripts/scaffold/features.json`).

### UI
5. Icons only via `@/components/icons` (generated from `components/icons/sets/*.ts`; add keys to your track's set). Never import `@tabler/icons-react` directly.
6. Page headers only through `PageContainer` / a page template. Forms through `useAppForm`. New shadcn components: `pnpm dlx shadcn@latest add …` in style `base-nova` (Base UI) — never mix in Radix variants. Base UI menus: `DropdownMenuLabel` must sit inside `DropdownMenuGroup`.
7. All user-facing strings through next-intl (`useTranslations` / `getTranslations`); messages live in the feature's `messages/{ru,en}.json` (namespace = feature id). `pnpm gen --check` fails on a key missing in either locale. Follow `docs/UX_WRITING.md`.
8. Status is never colour alone — icon + text (`SyncStatusBadge`, `RatingStars`). Use semantic tokens (`text-status-synced`, `bg-status-action-bg`, `text-rating-negative`, `text-platform-google`).
9. RBAC in the UI (`access` in `feature.ts`, `useCan`, `requireAccess`) is UX only; core-api is the authority.
10. Links styled as buttons: `LinkButton` (not `<Button render={<Link/>}>`).

### Parallel tracks (SDD-01T)
11. Work on a branch `ui/<track>/<slug>` in a worktree (`pnpm wt:new <track> <slug>`, `pnpm wt:dev`). Change only paths your track owns (`tracks.json`, `pnpm check:ownership`); other features are consumed via their `index.ts`; the public API of a feature changes only additively.
12. Need something from another track? Write `docs/requests/<date>-<from>-<slug>.md`; until it lands, a temporary copy may live in `features/<f>/components/_local/` with `// TODO(request: …)`.
13. Before pushing: `pnpm gen --check`, `pnpm typecheck`, `pnpm lint:strict`, `pnpm format:check`, `pnpm check:templates`, `pnpm depcruise`, `pnpm e2e` (your specs). `feature.ts` status: `planned` → `wip` (first PR) → `ready` (DoD in `docs/tracks/<track>.md`).

### Commands
```bash
pnpm mock:api            # mock core-api on MOCK_PORT (4010)
pnpm dev                 # next dev (CORE_API_URL from .env.local)
pnpm wt:dev              # both, on the worktree's ports
pnpm gen [--check]       # registries: messages, features, mocks, icons
pnpm scaffold:feature X  # skeleton of a feature (+ api from OpenAPI)
pnpm e2e | e2e:ui        # Playwright against the mock (prod build; E2E_DEV=1 for next dev)
pnpm check:templates | check:ownership | depcruise
```
Seed credentials: `owner@example.ru` / `password` (2FA user `admin@example.ru`, code `000000`); scenarios via `x-mock-scenario` header or cookie: `seed_default`, `empty_tenant`, `challenge_required`, `connector_degraded`.
