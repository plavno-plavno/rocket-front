...
import { OverviewPulse } from '@/features/overview/components/overview-pulse';
...
export default async function OverviewLayout({
  ...
  return (
    <PageContainer pageTitle={t('title')} pageDescription={t('description')}>
      <div className='flex flex-1 flex-col gap-5' data-template='analytics'>
        <OverviewHeaderActions />
        <OverviewPulse />
        <WelcomeWidget />
        <div className='grid gap-5 sm:grid-cols-2 xl:grid-cols-4'>{kpi}</div>
