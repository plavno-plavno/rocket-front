...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t('phrases.title') };
}

/** S-ANL-05 — owned by UI-F4 (renders `AnalyticsPage` via AnalyticsScreen). */
export default async function Page() {
  const access = await requireAccess({ permission: 'analytics.read' });
  return (
    <AnalyticsScreen screen='phrases' access={access}>
      <RankingTable dimension='phrases' />
    </AnalyticsScreen>
  );
}
