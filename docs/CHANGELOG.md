...
## e2e against the real core-api + contract 0.2.0-draft (2026-09-17)

- **`E2E_API_URL`** — `playwright.config.ts` runs the suite against a live core-api (`E2E_API_URL=http://localhost:3400/api/core` of the lp-core `--core` stack): the mock server is not started, the web app is built with that `CORE_API_URL`, and `global.setup.ts` resets the dataset through `POST {E2E_API_URL}/__dev/reset` (≈10 s truncate + seed) instead of `/__mock/reset`. Scenarios (`x-mock-scenario: empty_tenant`) are honoured by the real API's dev sign-in. The web server command is `pnpm build` / `pnpm dev` (pre-scripts included) so the Intl polyfills exist at build time — `next start` only serves `public/` files present when it was built.
- Import wizard e2e uses a real 10-row fixture (`e2e/fixtures/locations-import.csv`: 8 seeded branch codes, one new store, one broken row) so «создать 1 / обновить 8 / ошибок 1» comes from parsing, not from the mock's fabricated rows.
